...
function startGame() {
  foundWords = new Set();
  gameActive = false; // включится только после закрытия окна с правилами
  buildBoard();
  buildWordList();
  updateProgress();
  stopTimer();
...
// Закрытие окна правил: отсюда стартует таймер и включается ввод
function closeRulesAndStart() {
...
// ---------- Защита от случайного зума страницы ----------
// CSS touch-action блокирует pinch-жест на большинстве браузеров, но Safari
// на iOS дополнительно рассылает нативное событие gesturestart при защипе
// и может увеличить страницу при быстром двойном тапе рядом с кнопками —
// глушим оба сценария программно.
...
function init() {
  ...
  bindButton('rules-ok', closeRulesAndStart);

  bindButton('btn-game-menu', () => {
    document.getElementById('rules-overlay').classList.remove('visible');
    stopTimer();
    showScreen('menu');
  });
